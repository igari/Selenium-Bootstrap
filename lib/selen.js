...
Selen.prototype = {
  ...
  run: function (gen) {
    ...
    if(typeof gen === 'function') {
      ...
      return this.mocha.run(function(failures){
  ...
  describe: function (title, fnIncludedIt) {
    ...
    this.suite.beforeAll(this.start.bind(this));
    this.suite.afterAll(this.end.bind(this));
    fnIncludedIt();
  },
  ...
  runGen: function (gen, done) {
    ...
    function loop(promise) {
      promise.then((result) => {
        next = generator.next(result);   // nextの引数はyield式の評価結果として返される。そして次のyieldまで実行
        onEnd(next);
      }).catch((e) => {
