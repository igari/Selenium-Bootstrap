...
function Selen(capabilities, options) {
  ...
  let mochaOptions = Object.assign({
...
Selen.prototype = {
      ...
      return this.start()
        .then(function () {
          return new Promise(function (done) {
            ...
            return this.runGenerator(gen, done)
          }.bind(this))
        }.bind(this))
  ...
  it: function (title, gen) {
    ...
    this.suite.addTest(new Test(title, (done) => {
      this.runGenerator(gen.bind(this, this.driver, webdriver), done);
    }));
  },
...
v${packageJSON.version}
  ...
  saveScreenshot: function (imageFilePath) {
    ...
    return Promise.resolve()
      ...
      .then(function (currentUrl) {
        imageFilePath = imageFilePath || './selen-shots/' + this.getImageFileName(currentUrl);
          ...
          .then(function () {
            if (this.isMobileOS) {
              if (/android|safari/i.test(this.cap.browserName.toLowerCase())) {
                return capture.saveFullScreenshot(imageFilePath);
              } else {
                return capture.saveScreenshot(imageFilePath);
              }
            } else {
